...
`doodad place <x> <y>` 의 `(x, y)` 는 왼위가 아니라 **가운데**다.
6x6 두뎃을 (20, 20) 에 놓으면 **(17,17)~(22,22)** 를 덮는다. 찍어 보고
확인했다.

### 짝수 변이면 반 칸 밀린다

코드가 정수 나눗셈을 쓴다 (`map_archive.cpp` 의 `left = tileX - width/2`).

| 변의 길이 | `(20,20)` 에 놓으면 | 기하 중심 |
| --- | --- | --- |
| **홀수** (5x5) | 18~22 — **넣은 좌표가 정확히 가운데 칸** | **20.0** |
| **짝수** (6x6) | 17~22 — 가운데 두 칸 중 **오른쪽·아래쪽** | 19.5 |
| 홀수 (7x7) | 17~23 | **20.0** |
| 짝수 (8x8) | 16~23 | 19.5 |

DD2 에 적히는 픽셀 값은 정확한 중심이다
(`doodadCenterPixel(left, w) = left*32 + w*16`). 그래서 6x6 을
(20,20) 에 놓으면 픽셀은 656 — **반 칸 보정된 값**이 들어간다.

왕복(`doodadOriginTile`)이 홀짝을 나눠 처리하므로 **읽고 다시 써도
어긋나지 않는다.** 다만 **짝수 변 두뎃을 칸 단위로 정확히 가운데
맞추려 하면 반 칸이 어긋난다** — 대칭으로 놓을 때 걸린다.
...
두뎃 78 (6x6) 을 (20,20) 에 놓고 (16,16) 부터 읽은 그림
   ...
   .000000.......
   .0000RR.......
   .000RRR.......      ← (20,20) 은 여기 (짝수 변이라 기하 중심 19.5 보다 반 칸 오른쪽·아래)
   .000RR0.......
   .000000.......
```
