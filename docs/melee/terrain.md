...
래더·리그 맵을 세는 것만으로는 "어느 값을 어디에 놓아야 통하는가" 가 안 나와서,
...
> **에디터는 제대로 된 대칭 도구를 갖고 있다.** SCMDraft2 왼쪽의
> `Terrain Symmetry` 다 (평소에는 접혀 있다).
>
> | 도구 | 무엇 |
> | --- | --- |
> | `Mirror-X` · `Mirror-Y` · `Mirror-XY` | 거울 대칭. **지형 그릴 때 가장 많이 쓴다** |
> | **`Isom. Rot Symmetry`** | **마름모 회전 대칭.** 리마스터에서 들어왔다 |
> | `Tile Rot Symmetry` | 사각 지형 회전 대칭 |
>
> `Factor` 를 1~8 로 올리면 붓이 그만큼 늘어난다 (사각·육각·팔각 대칭).
> 두 회전 대칭은 **기준점이 다르다** — 마름모로 그릴 때는
> `Isom. Rot Symmetry` 를 쓴다
> ([기초) 지형 대칭 그리기](https://cafe.naver.com/edac/book5095400/66950)).
>
> **우리 CLI 에는 이 도구가 없다.** 그래서 아래처럼 붓질 좌표를 돌려
> 같은 일을 한다 — 타일을 베끼는 것이 아니라 **ISOM 붓을 대칭 자리에
> 다시 놓는다.**
