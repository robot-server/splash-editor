...
## 트리거로 **생성이 안 되는** 영웅이 있다

`Create Units` 액션으로 못 만드는 유닛이 있다. **초상화만을 위해
존재하는 더미 유닛**들이다 — 카페가 알다리스 · 듀갈 · 멩스크를 예로
든다
([[초급3] 액션](https://cafe.naver.com/edac/book5095361/76412)).

그래서 이 유닛들이 무기를 나눠 쓰는 것도 앞뒤가 맞는다. **더미라서
데이터를 따로 안 만든 것**이다 — 멩스크 · 노라드II · 듀갈이 무기 21번을
함께 쓰고, 타사다르(템플러)와 알다리스(템플러)가 69번을 함께 쓴다.

| 쓰려면 | |
| --- | --- |
| 맵에 **미리 놓는 것** | 된다 (에디터로 배치) |
| **트리거로 만드는 것** | **안 된다** |

유즈맵에서 "웨이브마다 보스를 만든다" 같은 구조에 이 유닛들을 넣으면
**아무것도 안 나온다.** 미리 놓고 `Give Units` 로 넘기거나,
`Remove`/`Move Unit` 으로 숨겼다 꺼내는 식으로 짠다.

> **아직 안 쟀다.** 어느 유닛까지 이에 해당하는지 목록으로 확인하지
> 못했다. 카페가 든 셋과 스캐럽 · 인터셉터는 확실하다. 쓰기 전에
> 맵에서 한 번 만들어 보고 확인한다.
