...
## 반복 트리거 기능을 epScript로 작성

- 패턴 이름: 클래식 트리거 조건·액션을 반복 호출하는 EUD 스크립트
- 구현하고자 하는 기능: 같은 액션을 플레이어·유닛마다 되풀이하거나, 클래식 트리거가 제공하지 않는 게임 상태 표시·메모리 조작을 구현한다.
- 조건(Condition) 및 액션(Action) 구조 체인: 초기화는 `onPluginStart()`에 둔다. 매 트리거 사이클 앞에 실행할 판정은 `beforeTriggerExec()`, 뒤에 실행할 작업은 `afterTriggerExec()`에 둔다. 각 함수 안에서 클래식 트리거 조건/액션을 호출하고, 한 번만 실행할 코드는 `once`로 감싼다. 반복 상태 변수는 선언 시 초기화한다.
- 예외 처리/버그 방지 로직: EUD 사용 전 [limits.md](limits.md)의 승인·버전 규칙을 따른다. 초기화하지 않은 변수는 이전 호출의 값이 남을 수 있다. `randomize()`를 호출하지 않으면 난수 시퀀스가 고정될 수 있다. 트리거 사이클 앞·뒤 어느 시점에 상태를 읽는지 명시하고, 클래식 트리거와 실행 순서를 확인한다.
