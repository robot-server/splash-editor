...
- 밀리맵은 맵마다 AI가 크기·본진/앞마당/멀티 자원·앞마당 거리·바깥 멀티 수·입구·추가 지형 패치 수/면적/종류·크리쳐 종류와 수·전투 구역 건축 불가 지형을 고르고 `make_melee.py` 옵션으로 명시한다. CLI 기본값을 설계 의도로 복사하지 않는다. 6인 이상은 128 타일 맵에서 자원 자리와 멀티가 겹치기 쉬워 더 큰 크기(예: 160x160)를 우선 검토한다. `--features`(0~64), `--feature-area`(8~384), 반복 `--feature-terrain`으로 AI가 고른 대칭 지형 덩이를 그리며 시작·자원 지대와 주요 진출로는 보전한다. 밀리 생성기의 Installation 제외는 설치본에서 본진·자원용 건축 가능 ISOM 바닥을 찾지 못한 측정에 따른다. `--main-entry auto`는 고지 경계를 찾아 타일셋에 맞는 후보를 `doodad fits`와 미니타일 연결로 검증하고, 최종 연결을 끊는 후보는 제거한 뒤 평지 입구로 복구한다. `--main-entry ramp`는 한 곳이라도 적합 램프를 못 놓으면 실패시킨다. `--main-entry bridge`는 현재 검증한 Jungle 타일셋의 2인 rot180 및 4인 rot90 템플릿에서만 사용하며, 양쪽 TILE/MTXM 패치와 두들 정렬·최종 시작 연결을 검사한다. 다른 타일셋/대칭은 요청을 거절한다. AI가 정한 `--fight-zone` 안의 walkable·건축 불가 지형은 맵 대칭에 맞춰 복제하고, 구역 타일 중 새로 추가된 비율이 8% 미만이면 실패시킨다. 이 지형은 유닛 길을 막지 않으며 `--max-unbuildable-pct` 전체 타일 상한으로 기지·멀티를 보전한다(기본 30%, 최대 45%). 중립 크리쳐는 `--critters` 수를 0~플레이어 수로 제한하고 종류는 맵마다 AI가 고른다. 생성 완료 뒤 자원 누락·요청한 멀티 링 누락·최종 스타팅 연결 실패가 있으면 실패 결과를 사용하지 않는다.
