...
def main(argv=None):
    ...
    n_up = scmap.setup_usemap_upgrades(
        ...
        free_levels=1, max_level=3,
        mineral=50, gas=0, time=12)
    n_tech = scmap.setup_usemap_tech(cli, which=(0, 3),
        mineral=75, gas=0, time=15, available="all")
    ...
    _nd = scmap.decorate_rim(cli, ts, [shelter, field, grave], rng, keep_clear=_clear)
    print(f"  두뎃 {_nd}개")
