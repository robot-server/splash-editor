...
> **현재 상태: M2 완료.**
> 맵을 열고, 메타데이터를 보고, 다시 저장할 수 있다. 재저장본이 게임에서
> 정상으로 열리는 것까지 확인했다. 지형 뷰·편집 기능은 아직 없다.
> 자세한 범위는 아래 [마일스톤](#마일스톤) 참고.
...
**크립 가장자리에 대하여.** StarCraft 의 크립은 가장자리가 유기적으로
들쭉날쭉하지만, 그 모양은 게임 데이터에 들어 있지 않다. 확인한 내용:

- `images.tbl` 929개 항목에 크립 관련 그래픽이 없다 (별도 스프라이트가 아니다)
- 타일셋의 크립 메가타일은 13종뿐이고 **전부 가득 찬 질감**이다
  (Jungle 기준 128~140번, 앞뒤는 풀·돌이다). 가장자리 전이 변형이 하나도 없다

즉 가장자리는 게임 엔진이 실행 중에 만들어 낸다. 타일셋 데이터만으로는
그 모양을 알 수 없으므로, 여기서는 경계를 좁게 페이드해 근사한다.
(`splash-cli images-tbl`, `mega-sheet`, `find-creep` 으로 직접 확인할 수 있다.)
